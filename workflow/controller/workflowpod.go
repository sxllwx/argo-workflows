		...
		if c.Name != common.WaitContainerName {
			...
			if len(c.Command) == 0 {
				x, err := woc.controller.entrypoint.Lookup(ctx, c.Image, entrypoint.Options{
					Namespace: woc.wf.Namespace, ServiceAccountName: woc.execWf.Spec.ServiceAccountName, ImagePullSecrets: woc.execWf.Spec.ImagePullSecrets, InsecureRegistry: woc.controller.insecureRegistry,
				})
				if err != nil {
